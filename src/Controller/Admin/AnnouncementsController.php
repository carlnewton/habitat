...
class AnnouncementsController extends AbstractController
{
    ...
    public function edit(
        ...
        if ('POST' === $request->getMethod()) {
            ...
            if (!empty($fieldErrors)) {
                return $this->render('admin/announcements.html.twig', [
                    ...
                    'values' => [
                        ...
                        'collapse' => $request->request->get('collapse'),
                        'type' => AnnouncementTypesEnum::from($request->request->get('type')),
                        'showDate' => $request->request->get('showDate'),
                        'hideDate' => $request->request->get('hideDate'),
                    ],
            ...
            $showDate = null;
            if (!empty($request->request->get('showDate'))) {
                $showDate = (new \DateTime())->setTimestamp(strtotime($request->request->get('showDate')));
            }

            $hideDate = null;
            if (!empty($request->request->get('hideDate'))) {
                $hideDate = (new \DateTime())->setTimestamp(strtotime($request->request->get('hideDate')));
            }

            $announcement
                ->setTitle(trim($request->request->get('title')))
                ->setContent(trim($request->request->get('content')))
                ->setCollapse(boolval($request->request->get('collapse')))
                ->setType(AnnouncementTypesEnum::from($request->request->get('type')))
    ...
    protected function validateRequest(Request $request): array
    {
        ...
        $showTimestamp = null;
        if (!empty($request->request->get('showDate'))) {
            $showTimestamp = strtotime($request->request->get('showDate'));
        ...
        $hideTimestamp = null;
        if (!empty($request->request->get('hideDate'))) {
            $hideTimestamp = strtotime($request->request->get('hideDate'));
