...
class UsersFreezeController extends AbstractController
{
    ...
    public function index(
        ...
        if (empty($request->request->get('freeze'))) {
        ...
        if (!empty($fieldErrors)) {
            return $this->render('admin/moderation/freeze_users.html.twig', [
                ...
                'errors' => $fieldErrors,
                'values' => [
                    'freezeForValue' => $request->request->get('freezeForValue'),
                    'freezeForInterval' => $request->request->get('freezeForInterval'),
        ...
        foreach ($users as $user) {
            ...
            $moderationLog
                ...
                ->setAction($this->translator->trans('moderation_log.actions.freeze', [
                    ...
                    '%freeze_value%' => $request->request->get('freezeForValue'),
                    '%freeze_interval%' => $request->request->get('freezeForInterval'),
                    '%reason%' => $request->request->get('reason'),
                ]))
            ;
            ...
            $freezeLog
                ...
                ->setUnfreezeDate((new \DateTimeImmutable())->modify(
                    '+' . $request->request->get('freezeForValue') .
                    ' ' . $request->request->get('freezeForInterval')
                ))
